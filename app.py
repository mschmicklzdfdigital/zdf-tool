...
# =========================================================
# EDITORIALER KATEGORIEN-KASTEN (NEUE POSITON)
# =========================================================
st.markdown("""
...
def categorize(title, url):
    t = title.lower().strip()
    u = url.lower().strip()
    if "wetter" in t or "gewitter" in t or "hitzewelle" in t or "unwetter" in t or "regen" in t: return "Gut zu wissen"
    pop_keywords = ["promi", "star", "heidi", "helene", "klum", "fischer", "lindenberg", "sänger", "schauspieler", "musik", "film", "serie", "show", "kino", "festival", "tiktok", "instagram", "viral", "kurios", "wunder", "hype", "popstar", "bühne", "oscar"]
    if "zdfheute.de/panorama/prominente" in u or any(x in t for x in pop_keywords): return "Trends, Pop & Kurioses"
    crime_keywords = ["mord", "tat", "gericht", "polizei", "kriminal", "prozess", "anklage", "festnahme", "ermittlung", "fahndung", "raub", "diebstahl", "schüsse", "toter", "leiche", "opfer", "täter", "haftbefehl", "sek", "clan", "drogen", "betrug", "jva", "gefängnis"]
    if any(x in t for x in crime_keywords): return "Zwischen Tat und Aufklärung"
    if "zdfheute.de/ratgeber" in u: return "Gut zu wissen"
    service_keywords = ["essen", "rezept", "ernährung", "gesundheit", "geld", "steuer", "miete", "rente", "tipps", "haushalt", "verbraucher", "krankenkasse", "gehalt", "sparen", "energie", "heizung", "urlaub", "reisen", "medizin", "arzt", "verkehr", "bahn"]
    ...
    analysis_keywords = ["analyse", "einordnung", "kommentar", "hintergrund", "erklärung", "zusammenhang", "politik", "bundestag", "scholz", "habeck", "baerbock", "regierung", "wahl", "usa", "trump", "biden", "china", "russland", "ukraine", "krieg", "sanktionen", "nato", "eu", "nahost", "israel", "gaza", "krise", "konflikt", "strategie", "reform", "podcasts"]
    if any(x in t for x in analysis_keywords): return "Macht und Folgen"
    return "Sonstige Artikel"

@st.cache_data(ttl=300)
def get_articles_from_rss():
    feeds = ["https://www.zdf.de/rss/zdf/nachrichten", "https://www.zdf.de/rss/zdf/politik", "https://www.zdf.de/rss/zdf/wirtschaft", "https://www.zdf.de/rss/zdf/panorama", "https://www.zdf.de/rss/zdf/sport", "https://www.zdf.de/rss/zdf/wissen"]
    fetched = []
    for feed in feeds:
        try:
            ...
            for item in root.findall(".//item"):
                ...
                if pub_date_str:
                    ...
                    except: article_time = datetime.now()
                else: article_time = datetime.now()
...
if file:
    ...
    excel_urls = set()
    if df.shape[1] > 1: excel_urls = set(df.iloc[:,1].astype(str).str.lower().str.strip())
    ...
    for cat in target_categories:
        ...
        if len(items) > 0:
            ...
            for item in items:
                st.markdown(f'<div class="news-card"><div class="news-headline">{item["title"].replace("\"", "&quot;")}</div><a href="{item["url"]}" target="_blank" class="news-url-anchor">{item["url"]}</a></div>', unsafe_allow_html=True)
    if not has_results: st.markdown('<div style="color:#2ed573; font-size:14px; font-weight:500; padding:20px 0;">✔ Alle Artikel wurden bereits über den WhatsApp-Kanal ausgespielt.</div>', unsafe_allow_html=True)
